...
static  void  NetIF_Pkt_Rx (void)
{
    ...
    /*start 2013-12-20 modified by xjx, */
    size = 1514;//NET_BUF_CFG_DATA_SIZE_LARGE;//NetNIC_RxPktGetSize();                         /* Get pkt size.                                        */
    /*end 2013-12-20 modified by xjx*/
    size_valid = NetIF_IsValidPktSize(size);
    if (size_valid != DEF_YES) {
        //Discard¨®|??¨º???NIC buffer?D¨ºy?Y?¨¢3?¡ê?¨®¨¦¨®¨²2??¨¢3???Size¨°¨¬3¡ê?¨¦??¡ê??¨´¨°???2?¡ä|¨¤¨ª
    ...
    switch (err_rx) {
        case NET_BUF_ERR_NONE:                                  /* If NO errs, buf avail;    ...                        */
             ...
             rev_size = NetNIC_RxPkt((void *)p_data, size, &err_rx);       /* ...       & rd data from NIC into buf.               */
             break;
...
/*$PAGE*/
    ...
    switch (err_rx) {                                           /* Chk err from NetNIC_RxPkt().                         */
        case NET_NIC_ERR_NONE:
             pbuf_hdr                  = &pbuf->Hdr;
             pbuf_hdr->TotLen          = (NET_BUF_SIZE)rev_size;    /* Set pkt size as buf tot len & data len.              */
